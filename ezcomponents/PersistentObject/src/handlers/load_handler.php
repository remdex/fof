...
class ezcPersistentLoadHandler extends ezcPersistentSessionHandler
{
    ...
    public function loadIntoObject( $object, $id )
    {
        ...
        if ($this->database instanceof ezcDbHandlerMongoDb) {
               
            $row = $this->database->dbInstance->{$def->table}->findOne($q->expr->eqId(
    ...
    public function find( $query, $class = null )
    {
        ...
        if ($this->database instanceof ezcDbHandlerMongoDb) {
            $rowsCursor = $this->session->performQuery( $query ); 
            $rows = iterator_to_array($rowsCursor);
            ...
            foreach ( $rows as $row )
            {
                ...
                if (is_object($row[$def->idProperty->resultColumnName])) {
                    $result[(string)$row[$def->idProperty->resultColumnName]] = $object;
                } else {
