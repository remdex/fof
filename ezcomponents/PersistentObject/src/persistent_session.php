...
class ezcPersistentSession implements ezcPersistentSessionFoundation
{
     ...
     */
    public function __construct( $db, ezcPersistentDefinitionManager $manager, $mongoHandler = false )
    {
        ...
        $this->properties['mongoHandler']      = $mongoHandler;
    }
     ...
     * @access private
     */
    public function performQuery( $q, $transaction = false )
    {
        if ( $this->properties['mongoHandler'] == true ) {
            $stmt = $q->execute();
            return $stmt;
        }
