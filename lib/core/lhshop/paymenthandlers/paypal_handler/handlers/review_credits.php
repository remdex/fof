...
$tpl = erLhcoreClassTemplate::getInstance( 'lhshop/handlers/paypal_handler/review_credits.tpl.php');
...
if (isset($_POST['ConfirmPayment']))
{
	...
	if (round((float)$paymentAmount,2) == round((float)$basket->amount,2))
	{		
		...
		switch($ack) {
		  ...
		  case 'SuccessWithWarning':
		  	...
		  	$credits->credits += $basket->credits;
		  	
			$basket->status = erLhcoreClassModelShopUserCreditOrder::ORDER_STATUS_CONFIRMED ;
			$basket->date = time() ;
		  	erLhcoreClassShopMail::sendOrderCreditsMail($basket);
...
$tpl->set('credits',$basket->credits);
