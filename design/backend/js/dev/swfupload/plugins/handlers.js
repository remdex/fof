...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			progress.setStatus(translations.fileistobig);
...
function uploadStart(file) {
	try {
		 ...
		 */
		var progress = new FileProgress(file, this.customSettings.progressTarget,this);
		progress.setStatus(translations.uploading+"...");
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.setStatus(translations.complete+'.');
		progress.toggleCancel(false);
...
// Jei visi failai baigti pazymeti sesija kaip baigta ir paruosta konvertavimui.
// Pazymim kad failu siuntimo sesija paruosta.
function uploadComplete(file) {
	if (this.getStats().files_queued === 0) {	    
	    fileconverter.flashQueCompleted(); 
	}
}
