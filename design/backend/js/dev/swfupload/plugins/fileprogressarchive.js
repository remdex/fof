...
function FileProgress(file, targetID, swfUploadInstance) {
	...
	if (!this.fileProgressWrapper) {
		...
		$.postJSON(this.wwwDir + 'gallery/fileuploadcontainerarchive/'+this.fileProgressID,{'filename':fileName},function(data){
...
FileProgress.prototype.attatchActions = function()
{
	...
	swfUploadInstance.addFileParam(fileID,'album_id',$('#AlbumIDToUploadArchive'+this.fileProgressID).val());
	
	
	$('#PhotoTitle'+this.fileProgressID).change(function(){
	    swfUploadInstance.addFileParam(fileID,'title',$(this).val())
	    if ($(this).val() != '') {
	    	$('#AlbumIDToUploadArchive'+fileID).attr('disabled','disabled');
	    } else {
	    	$('#AlbumIDToUploadArchive'+fileID).removeAttr('disabled');
	    }
	});
	...
	$('#AlbumIDToUploadArchive'+this.fileProgressID).change(function() {
		
		if ($(this).val() > 0) {
			$('#PhotoTitle'+fileID).attr('disabled','disabled');
			$('#PhotoKeyword'+fileID).attr('disabled','disabled');
			$('#PhotoDescription'+fileID).attr('disabled','disabled');
		} else {
			$('#PhotoTitle'+fileID).removeAttr('disabled');
			$('#PhotoKeyword'+fileID).removeAttr('disabled');
			$('#PhotoDescription'+fileID).removeAttr('disabled');
		}
		
	    swfUploadInstance.addFileParam(fileID,'album_id',$(this).val())
	});
